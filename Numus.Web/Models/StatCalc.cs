...
public class StatCalc
{
    ...
    public StatCalc(string address,
        BigInteger balance,
    ...
    public WalletStats GetStats()
    {
        ...
        var buyTokens = _tokenTransfers.Where(x => x.To == _address && soldTokensIds.Contains(x.GetTokenUid()));
        var buySum = GetTokensSum(buyTokens);

        var buyNotSoldTokens = _tokenTransfers.Where(x => x.To == _address && !soldTokensIds.Contains(x.GetTokenUid()));
